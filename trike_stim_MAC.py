...
def getEulerAngles():
    global run, readingAngles, usingPort, channels, pulse_width, old_current, old_pulse_width, old_signal_femoral, old_signal_gastrocnemius, current
...
#                signal_femoral.append((femoral(phase(ang))+signal_speed_actual[-1])*signal_speed_actual[-1])
                ...
                if (old_signal_femoral != signal_femoral[-1]) | (old_signal_gastrocnemius != signal_gastrocnemius[-1]):
...
#                    print botao
            if (int(botao) == 1) & (not readingAngles):
                ...
                with open("time", 'w') as f:
                    for s in time_stamp:
                        f.write(str(s) + '\n')
                with open("control_1", 'w') as f:
                    for s in signal_femoral:
                        f.write(str(s*femoral_max) + '\n')
                with open("control_2", 'w') as f:
